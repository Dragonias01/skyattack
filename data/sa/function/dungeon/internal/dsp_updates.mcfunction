...
# --- AKTIV ---
    $execute if score server dg$(nr)_state matches 1..4 run function sa:dungeon/internal/display/update_players
...
$execute as @a[tag=dg$(nr)_in_dungeon] in sa:dungeon1 run scoreboard players add server dg$(nr)_players_dungeon 1
$execute as @a[tag=dg$(nr)_in_boss] in sa:dungeon_arena_1 run scoreboard players add server dg$(nr)_players_boss 1
