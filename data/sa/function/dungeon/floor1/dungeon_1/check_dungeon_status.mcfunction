...
# bei status 0 = Check ob item in inventory -> open -> status 1
    ...
    execute as @s if score server dg1_state matches 0 if score @s temp matches 1.. run function sa:dungeon/internal/open with storage sa:config dungeon.floor1.dungeon1
