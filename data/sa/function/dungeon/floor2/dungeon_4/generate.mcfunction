# ============================================================================
# DUNGEON 2 - GENERATE
# Wrapper für altes Random Dungeon System
# ============================================================================

execute if score server dg4_generated matches 1 run return 1

# Random coords generieren
    execute store result score #x temp run random value -10000..10000
    execute store result score #z temp run random value -10000..10000


execute store result storage sa:dungeon_4_coords x int 1 run scoreboard players get #x temp
execute store result storage sa:dungeon_4_coords z int 1 run scoreboard players get #z temp


# Dimension vorbereiten
    execute in sa:dungeon4 run function sa:dungeon/floor2/dungeon_4/clear_dimension

# Dungeon generation scheduled 
    schedule function sa:dungeon/floor2/dungeon_4/generate_dungeon 5t

# Flags setzen
    scoreboard players set server dg4_generated 1
    scoreboard players set server dg4_state 2
