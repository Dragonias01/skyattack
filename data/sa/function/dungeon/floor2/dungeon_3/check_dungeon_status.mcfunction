...
# bei status 0 = Check ob item in inventory -> open -> status 1
    ...
    execute as @s if score server dg3_state matches 0 if score @s temp matches 1.. run function sa:dungeon/internal/open with storage sa:config dungeon.floor2.dungeon3
    execute as @s if score server dg3_state matches 0 unless score @s temp matches 1.. run tellraw @s {"text":"You need a Dungeon Key to enter!","color":"red"}
    scoreboard players reset @s temp

# bei status 1-4 = join function

execute if score server dg3_state matches 1.. run function sa:dungeon/internal/join_dungeon with storage sa:config dungeon.floor2.dungeon3
